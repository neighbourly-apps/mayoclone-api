...
class ReportSummaryIntegrationTest extends AbstractIntegrationTest {
    ...
    void summaryMathAndZeroFill() throws Exception {
        ...
        // Day 1 — two fulfilled (BILL_PENDING) ZOOP orders + one cancelled DIRECT/COD.
        seed(accountId, zoop, PaymentMode.PREPAID, OrderStatus.BILL_PENDING, "300", D1, "12951", "NDLS",
                30, List.of(item("Biryani", 3, "100")));
        seed(accountId, zoop, PaymentMode.PREPAID, OrderStatus.BILL_PENDING, "200", D1, "12951", "NDLS",
                60, List.of(item("Dosa", 1, "100"), item("Biryani", 1, "100")));
        seed(accountId, null, PaymentMode.COD, OrderStatus.CANCELLED, "150", D1, "12303", "ALD",
                null, List.of(item("Tea", 3, "50"))); // excluded from revenue + topItems
        // Day 2 — one cancelled DIRECT/COD + one NEW DIRECT/PREPAID.
        ...
        mvc.perform(get("/api/reports/summary?from=" + FROM + "&to=" + TO)
                        ...
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                // totals: 5 orders, revenue excludes the two CANCELLED (150 + 500) -> 600
                .andExpect(jsonPath("$.totals.orders").value(5))
                .andExpect(jsonPath("$.totals.revenue").value(600))
                // paymentSplit: online = PREPAID (3 orders, 600), cod = COD (2 cancelled orders, 0)
                ...
                .andExpect(jsonPath("$.paymentSplit.cod.revenue").value(0))
                // statusCounts (all 5 present)
                .andExpect(jsonPath("$.statusCounts.NEW").value(1))
                .andExpect(jsonPath("$.statusCounts.BILL_PENDING").value(2))
                .andExpect(jsonPath("$.statusCounts.CANCELLED").value(2))
                .andExpect(jsonPath("$.statusCounts.ACCEPTED").value(0))
                .andExpect(jsonPath("$.statusCounts.OUT_FOR_DELIVERY").value(0))
                // fulfillmentRate = BILL_PENDING / (BILL_PENDING + CANCELLED) = 2 / (2 + 2) = 0.5
                .andExpect(jsonPath("$.fulfillmentRate").value(org.hamcrest.Matchers.closeTo(0.5, 0.001)))
                // avgDeliveryMinutes over BILL_PENDING = (30 + 60) / 2 = 45
                ...
                .andExpect(jsonPath("$.byDay[0].revenue").value(500))
                ...
                // topItems by qty (non-cancelled): Biryani 4 / rev 400, Dosa 2 / 200 (Tea only in cancelled -> absent)
                ...
                .andExpect(jsonPath("$.topItems[1].name").value("Dosa"))
                .andExpect(jsonPath("$.topItems[1].qty").value(2))
